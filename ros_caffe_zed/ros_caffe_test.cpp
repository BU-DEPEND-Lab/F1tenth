...
std::vector<Prediction> predictions = classifier->Classify(img);
        float end = clock();
        float dur = (end - begin) / CLOCKS_PER_SEC ;        
    for (size_t i = 0; i < predictions.size(); ++i) {
        Prediction p = predictions[i];
        std::cout << std::fixed << std::setprecision(4) << p.second << " - \"" << p.first << "\"" << dur << "second"<<std::endl;
    }
...
int main(int argc, char **argv) {
    ...
    float time2 = clock();
    float dur = time2 - time1;
    std::cout<< dur << std::endl;
    image_transport::Subscriber sub = it_zed.subscribe(RECEIVE_IMG_TOPIC_NAME, 1, imageCallback);
