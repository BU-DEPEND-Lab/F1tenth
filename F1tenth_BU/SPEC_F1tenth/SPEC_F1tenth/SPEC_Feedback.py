...
def scenario_optimization(car, hj, num=100):
    ...
    for i in range(num):
        ...
        hj.get_value_map(0.5, dis_barrel, dis_wall)
        traj, _, traj_tau, _ = hj.compute_traj(xinit)
        print traj.shape
        traj_u, failure = hj.compute_ctrl(xinit)
        if hj.empty_flag:
            for idx, tau in enumerate(traj_tau[0]):
                car.step(traj_u)
