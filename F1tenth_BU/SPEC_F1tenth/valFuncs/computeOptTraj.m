function [traj, traj_u, traj_tau] = computeOptTraj(g, data, tau, dynSys, extraArgs)
...
traj_u = nan(dynSys.nu, tauLength*subSamples);
traj_u(:,1:subSamples) = zeros(dynSys.nu, subSamples);
tEarliest = 1;
...
traj(:,iter:end) = [];
traj_u(:,(iter-1)*subSamples+1:end) = [];
